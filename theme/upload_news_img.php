...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	$sql = "UPDATE venue_database.news SET news_img = " . $file_name_quotes . " WHERE ID = " . $rid[$i];
	...
	$img_paths = array();
	$sql = "SELECT news_img FROM venue_database.news WHERE feature_img = 1";
	...
	if($stmt = $db->query($sql))
	{
		...
		{
			echo $row["news_img"];
			array_push($img_paths, $row["news_img"]);
		}	
	}
	else 
	{
